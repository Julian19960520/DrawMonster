...
import { BgData, ThemeData } from "../../Frame/dts";
...
@ccclass
export default class BgEditCell extends BaseEditCell {
    data:BgData = null;
    ...
    setData(data:BgData) {
        ...
        if(!data["createNew"]){
            Game.loadTexture(data.url, "bg", (texture)=>{
                let frame = new cc.SpriteFrame();
                frame.setTexture(texture);
                this.sprite.spriteFrame = frame;
                this.sprite.node.color = data.color || cc.Color.WHITE;
            });
            let themeId  = DB.Get(Key.ThemeId);
            let theme = Game.findThemeConf(themeId);
            this.setUsingState(theme.bgId == data.id);
        }
    }
    ...
    onClick(){
        ...
        if(this.data["createNew"]){
            ...
            theme.bgId = this.data.id;
            DB.Save(Key.CustomThemes);
            this.node.dispatchEvent(Util.customEvent("updateBg", true));
            this.node.dispatchEvent(Util.customEvent("updateBg",true));
        }
    }
