...
import { ThemeData, ShieldData } from "../../Frame/dts";
...
@ccclass
export default class ShieldEditCell extends BaseEditCell {
    data:ShieldData = null;
    ...
    setData(data:ShieldData) {
        ...
        if(!data["createNew"]){
            Game.loadTexture(data.url,"shield",(texture)=>{
            ...
            this.setUsingState(theme.shieldId == data.id);
        }
    }
    ...
    onClick(){
        ...
        if(this.data["createNew"]){
            ...
            theme.shieldId = this.data.id;
            DB.Save(Key.CustomThemes);
            this.node.dispatchEvent(Util.customEvent("updateShield", true));
        }
    }
